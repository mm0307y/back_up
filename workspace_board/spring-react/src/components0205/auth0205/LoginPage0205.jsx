...
import { DividerDiv, DividerHr, DividerSpan, GoogleButton, KakaoButton, LoginForm, MyH1, MyInput, MyLabel, MyP, PwEye, SubmitButton } from '../../styles0210/FormStyles'
import { Link, useNavigate } from 'react-router'
import { loginGoogle } from '../../service0205/authLogic0212'
import { useDispatch, useSelector } from 'react-redux'
import { setToastMsg } from '../../redux0212/toastStatus0212/action0212'
...
const LoginPage0205 = () => {
  ...
  const loginG = async () => {
    try {
      ...
      // props로 받지 않고 리덕스 store를 통해서 초기화 된 정보를 어디서나 바로 사용이 가능하다. - redux
      // 아래 함수에 파라미터 자리의 값은 리덕스의 store에 있는 정보를 읽어 온다.
  ...
  const loginK = () => {
    try {
      // 카카오 인증 코드 가져오기
      const kakaoAuthUrl = 'https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=4ea7ace8a82124712ffdfd528e65bcf5&redirect_uri=http://localhost:8000/auth/kakao/callback'
      window.location.href = kakaoAuthUrl // 브라우저로 리다이렉션 발생 - 카카오 서버에 요청이 전달 된다.
